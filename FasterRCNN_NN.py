...
import numpy as np
from NN_Helper import gen_train_target, gen_candidate_anchors, bbox_tools
...
class FasterRCNN():
    def __init__(self):
        ...
        self.anchor_candidate_generator = gen_candidate_anchors()
    ...
    def test_proposal_visualization(self):
        ...
        RPN_Anchor_Pred = tf.slice(RPN_Anchor_Pred, [0,0,0,0,1], [1,23,40,9,1])
        print(RPN_Anchor_Pred.shape, RPN_BBOX_Regression_Pred.shape)
        ...
        original_indices = tf.where(tf.equal(RPN_Anchor_Pred, 2))
        DebugPrint('original_indices shape', original_indices.shape)
        DebugPrint('original_indices', original_indices)
        base_boxes = tf.gather_nd(self.anchor_candidates, original_indices)
        DebugPrint('base_boxes shape', base_boxes.shape)
        DebugPrint('base_boxes', base_boxes)
        base_boxes = np.array(base_boxes)


        final_box = bbox_tools.bbox_reg2truebox(base_box=base_boxes, reg=final_box_reg)

        # clip the boxes to make sure they are legal boxes
        x_max, y_max = 720, 1280
